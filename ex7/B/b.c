...
void xenTaskA(void *arg){
	...
	//Timed start
	//rt_mutex_acquire(&resource_m,TM_INFINITE);
	rt_sem_wait(&resource,TM_INFINITE);
	print_pri(&task1,"[A] low pri task locked resource\n");
	//print_pri(&taskd,"taskD pri\n");
	//print_pri(&taskb,"taskB pri\n");
	busy_wait_ms(3);
	print_pri(&task1,"Task A now releasing resource\n");
	//rt_mutex_release(&resource_m);
...
void xenTaskB(void *arg){
	...
	//Timed start
	rt_task_sleep_ms(1);
	print_pri(&taskb,"[B] Mid pri Busy waiting\n"); 
	//print_pri(&taskd,"taskD pri\n");
	//print_pri(&task1,"taskA pri\n");
	busy_wait_ms(5);
	rt_printf("Task B now finnished\n");
}
void xenTaskD(void *arg){
	rt_printf("Task D now waiting for semaphore\n");
	rt_sem_wait(&awesomeSem, TM_INFINITE);
	...
	//rt_mutex_acquire(&resource_m,TM_INFINITE);
	rt_sem_wait(&resource,TM_INFINITE);
	print_pri(&taskd,"[D] High pri task locked resource and running\n");
	//print_pri(&task1,"taskA pri\n");
	//print_pri(&taskb,"taskB pri\n");
	busy_wait_ms(2);
	rt_printf("Task D now releasing resource\n");
	//rt_mutex_release(&resource_m);
	rt_sem_signal(&resource);
...
int main(int argc, char **argv){
	...
	int disturbance=1;
	rt_mutex_create(&resource_m,"resource");
